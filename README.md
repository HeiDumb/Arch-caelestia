...
- `home/.local/bin/` contains launch wrappers that the Hyprland and systemd configs call
...
For the friendly path, run the guided installer:

```bash
bash setup/bootstrap.sh
```

It will ask before each phase. For package lists and services you can choose `all`, `none`, `view`, `edit`, or numbered selections like `1-5,9,12`.

If you want the no-questions fast path, run everything:
...
- `sync` copies the curated `home/` files into place without deleting existing local files
- `dconf` restores exported desktop preferences
- `services` reloads and enables the exported user services

If you want an exact mirror, run `bash setup/bootstrap.sh sync-clean`. That mode uses `rsync --delete`, but moves removed or replaced files into `~/.rice-sync-backups/` first.
...
To publish the first copy to GitHub after creating the empty repo there:

```bash
git remote add origin git@github.com:HeiDumb/rice.git
git push -u origin main
```

## Helpful Tips

- Use a dry run before syncing if you are restoring onto a system that already has your own files: `bash setup/bootstrap.sh sync-dry-run`
